...
from eval.latency import latency_tracker, measure_vector_retrieval
...
class VectorDbManager:
    ...
    def get_collection(self, collection_name) -> QdrantVectorStore:
        """Get collection with latency tracking for retrieval operations."""
        try:
            ...
            # Wrap the similarity_search method to track retrieval latency
            original_similarity_search = collection.similarity_search
            
            def tracked_similarity_search(*args, **kwargs):
                with latency_tracker.measure_operation('vector_retrieval', {
                    'collection_name': collection_name,
                    'k': kwargs.get('k', 3),
                    'score_threshold': kwargs.get('score_threshold', None)
                }):
                    return original_similarity_search(*args, **kwargs)
            
            collection.similarity_search = tracked_similarity_search
            return collection
