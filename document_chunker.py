...
from eval.latency import latency_tracker, measure_document_ingestion
...
class DocumentChuncker:
    ...
    def create_chunks(self, path_dir=config.MARKDOWN_DIR):
        """Create chunks from documents with latency tracking."""
        metadata = {
            'directory': path_dir,
            'file_count': len(glob.glob(os.path.join(path_dir, "*.md")))
        }
        
        with latency_tracker.measure_operation('document_chunking', metadata):
